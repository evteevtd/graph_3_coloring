...
os.system(f"./build/{lower_bound} {filename} > results/{lower_bound}_{graph_name}_result")

print(f"seed = {seed}")

with open(f"results/{upper_bound}_{graph_name}_result", 'r') as ub_res:
    print(f"upped_bound_result: {ub_res.readline()}")

with open(f"results/{lower_bound}_{graph_name}_result", 'r') as lb_res:
